...
STOP_POINTS = 22.0   # $220 per contract — optimal from stop scan
...
def main():
    ...
    for n in CONTRACT_SIZES:
        ...
        trades_df.to_csv(
            RESULTS_DIR / f"mgc_htc22_{n}ct_{timestamp}.csv", index=False)
        daily_df.to_csv(
            RESULTS_DIR / f"mgc_htc22_daily_{n}ct_{timestamp}.csv", index=False)
    ...
    if safe:
        ...
        best_trades = pd.read_csv(
            sorted(RESULTS_DIR.glob(f"mgc_htc22_{best_n}ct_{timestamp}.csv"))[-1]
        )
